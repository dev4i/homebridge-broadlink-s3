...
export class ExampleHomebridgePlatform implements DynamicPlatformPlugin {
   ...
   */
	discoverDevices() {
		...
		const pythonProcess = spawn('python3', [ 'src/get-all-subdevices.py' ], { shell: true });

		pythonProcess.stdout.on('data', (data) => {
			console.log('Python output:');
			console.log(data.toString());

			var allSubdevices = JSON.parse(data.toString()).map((dev) => {
			...
			for (const device of allSubdevices) {
				...
				if (existingAccessory) {
					...
					if (device.deviceName === 'simpfc_cli') {
						...
						const accessory = new this.api.platformAccessory('LC1 Switch 3 Gang', uuid);
